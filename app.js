...
const MAPS = [
  { id: 'inferno', name: 'Inferno', image: 'maps/inferno.webp', logo: 'maps/inferno.webp', background: 'maps/backgroundmaps/Cs2_inferno_remake.webp', description: 'Banana pegando fogo, retake sofrido e a clÃ¡ssica trocaÃ§Ã£o no bomb A.' },
  ...
  { id: 'nuke', name: 'Nuke', image: 'maps/nuke.webp', logo: 'maps/nuke.webp', background: 'maps/backgroundmaps/Cs2nuke.webp', description: 'Vertical, tÃ¡tico e perfeito para fake, vent dive e call torta.' },
  { id: 'ancient', name: 'Ancient', image: 'maps/ancient.webp', logo: 'maps/ancient.webp', background: 'maps/backgroundmaps/De_ancient_cs2.webp', description: 'Pedra, utilitÃ¡ria pesada e avanÃ§o pressionando caverna o tempo todo.' },
  { id: 'anubis', name: 'Anubis', image: 'maps/anubis.webp', logo: 'maps/anubis.webp', background: 'maps/backgroundmaps/Anubis1.webp', description: 'Ãgua, entrada forte e rounds rÃ¡pidos com muita pressÃ£o de mapa.' },
  { id: 'dust2', name: 'Dust II', image: 'maps/dust2.webp', logo: 'maps/dust2.webp', background: 'maps/backgroundmaps/Cs2_dust2.webp', description: 'O mapa mais clÃ¡ssico da rotaÃ§Ã£o, com ego, pixel e bala cantando.' },
  { id: 'overpass', name: 'Overpass', image: 'maps/overpass.webp', logo: 'maps/overpass.webp', background: 'maps/backgroundmaps/Overpass_loading_screen.webp', description: 'Controle de mapa, banheiro brigado e clutch nas rotaÃ§Ãµes longas.' }
];
...
function goToNextMap() {
  ...
  updateMapBanner();
  setSummary(`PrÃ³ximo mapa selecionado: ${MAPS[nextIndex].name}.`);
}
...
function parseSteamXmlProfile(xmlText, fallbackUrl) {
  ...
  if (!personaName) throw new Error('Perfil Steam sem nome visÃ­vel.');
  return { personaName, steamId64, avatar, steamUrl: normalizeSteamUrl(steamUrl || fallbackUrl) };
}

async function fetchWithTimeout(resource, options = {}) { const { timeout = STEAM_REQUEST_TIMEOUT, ...rest } = options; const controller = new AbortController(); const timeoutId = setTimeout(() => controller.abort(), timeout); try { return await fetch(resource, { ...rest, signal: controller.signal }); } finally { clearTimeout(timeoutId); } }
async function resolveSteamProfile(profileUrl) { const parsed = parseSteamProfileUrl(profileUrl); if (!parsed) throw new Error('Use um link vÃ¡lido do perfil Steam (/id/ ou /profiles/).'); const fallback = getFallbackProfile(parsed.url); if (fallback) return fallback; const xmlUrl = `${parsed.url}?xml=1`; let lastError = null; for (const proxy of STEAM_XML_PROXIES) { for (let attempt = 0; attempt < STEAM_PROXY_RETRY_COUNT; attempt += 1) { try { const response = await fetchWithTimeout(proxy.buildUrl(xmlUrl), { headers: { Accept: 'application/xml,text/xml;q=0.9,*/*;q=0.8' } }); if (!response.ok) throw new Error(`${proxy.name} respondeu ${response.status}`); const xmlText = await response.text(); if (!xmlText || !xmlText.includes('<profile>')) throw new Error(`${proxy.name} nÃ£o retornou XML Steam vÃ¡lido.`); return parseSteamXmlProfile(xmlText, parsed.url); } catch (error) { lastError = error; } } } throw new Error(lastError?.message || 'NÃ£o foi possÃ­vel carregar o perfil Steam agora.'); }

function ensureSteamPreview(profile) { steamPreviewAvatar.src = profile.avatar || 'logo.png'; steamPreviewName.textContent = profile.personaName || 'Perfil encontrado'; steamPreviewUrl.textContent = profile.steamUrl || ''; steamPreview.classList.remove('hidden'); }
function resetSteamPreview() { steamPreviewData = null; steamPreview.classList.add('hidden'); steamPreviewAvatar.src = ''; steamPreviewName.textContent = 'Perfil encontrado'; steamPreviewUrl.textContent = ''; }
...
function getPlayerStatsHtml(name) { const stats = getPlayerStats(name); return `<div class="player-stat-chips"><span class="player-stat-chip">${stats.wins} vitÃ³rias</span><span class="player-stat-chip">${stats.losses} derrotas</span><span class="player-stat-chip">${stats.points} pontos</span></div>`; }
...
async function updatePlayerSkill(name, newSkill) { const player = getPlayerRecord(name); if (!player) return false; player.skill = normalizeSkill(newSkill); await saveState(); addAdminLog('skill', `CSficaÃ§Ã£o de ${player.name} atualizada para ${player.skill}.`, { player: player.name, skill: player.skill }); renderAll(); return true; }
...
async function saveSkillUpdate() { if (!editingSkillPlayer) return; const newSkill = normalizeSkill(editSkillInput.value); await updatePlayerSkill(editingSkillPlayer, newSkill); closeSkillModal(); setSummary(`CSficaÃ§Ã£o de ${editingSkillPlayer} atualizada para ${newSkill.toLocaleString('pt-BR')}.`); }
...
function renderDatabasePlayers() {
  ...
  if (!state.playersDatabase.length) { databasePlayerList.innerHTML = '<li class="empty-state">Nenhum jogador cadastrado ainda.</li>'; return; }
  const adminVisible = isAdminLogged();
  databasePlayerList.innerHTML = state.playersDatabase.map((player) => `<li class="name-item db-item"><div class="db-player-main"><img class="player-avatar" src="${escapeHtml(player.avatar || 'logo.png')}" alt="${escapeHtml(player.name)}" /><div class="db-player-meta"><strong>${escapeHtml(player.name)}</strong><span class="db-skill">CSficaÃ§Ã£o: ${normalizeSkill(player.skill).toLocaleString('pt-BR')}</span>${getPlayerStatsHtml(player.name)}</div></div><div class="db-actions"><button class="small-action add-db-btn" data-add-db="${escapeHtml(player.name)}">+</button><button class="small-action edit-skill-btn" data-edit-skill="${escapeHtml(player.name)}">âœŽ</button>${adminVisible ? `<button class="small-action remove-db-btn" data-remove-db="${escapeHtml(player.name)}">Ã—</button>` : ''}</div></li>`).join('');
}
function renderPlayers() { playerCount.textContent = `${players.length} / 10`; if (!players.length) { playerList.innerHTML = '<li class="empty-state">Nenhum jogador no lobby ainda.</li>'; return; } playerList.innerHTML = players.map((name) => `<li class="name-item"><div class="lobby-player-main"><img class="lobby-avatar" src="${escapeHtml(getPlayerAvatar(name))}" alt="${escapeHtml(name)}" /><div class="db-player-meta"><strong>${escapeHtml(name)}</strong><span class="db-skill">CSficaÃ§Ã£o: ${getPlayerSkill(name).toLocaleString('pt-BR')}</span></div></div><button class="remove-btn" data-remove-lobby="${escapeHtml(name)}">Ã—</button></li>`).join(''); }
...
function startMatch() { if (!currentDraw) { setSummary('Sorteie os times antes de comeÃ§ar a partida.'); return; } currentMatch = { ...currentDraw, startedAt: new Date().toISOString() }; updateStatus('Partida em andamento'); setSummary(`Partida iniciada em ${currentMatch.mapName}.`); }
...
async function handleSteamRegister() { const steamUrl = normalizeSteamUrl(steamProfileInput.value); const skill = normalizeSkill(playerSkillInput.value); if (!steamUrl) { setSummary('Cole um link vÃ¡lido do perfil Steam.'); return; } addPlayerBtn.disabled = true; addPlayerBtn.textContent = 'Carregando...'; try { const profile = await resolveSteamProfile(steamUrl); steamPreviewData = profile; ensureSteamPreview(profile); const action = await addPlayerToDatabase(profile, skill); renderDatabasePlayers(); renderRanking(); setSummary(action === 'updated' ? `${profile.personaName} jÃ¡ existia e foi atualizado.` : `${profile.personaName} foi cadastrado globalmente.`); steamProfileInput.value = ''; playerSkillInput.value = ''; } catch (error) { setSummary(error.message || 'Falha ao carregar o perfil Steam.'); } finally { addPlayerBtn.disabled = false; addPlayerBtn.textContent = 'Cadastrar'; } }

function bindEvents() { if (toggleDatabasePanelBtn) toggleDatabasePanelBtn.addEventListener('click', toggleDatabasePanel);
  ...
  rerollBtn.addEventListener('click', () => drawTeams(currentDrawMode));
  addAfterDrawBtn.addEventListener('click', () => setSummary('Adicione jogadores no lobby pelos cards do cadastro.'));
  ...
  databasePlayerList.addEventListener('click', (event) => { const addButton = event.target.closest('[data-add-db]'); const removeButton = event.target.closest('[data-remove-db]'); const editSkillButton = event.target.closest('[data-edit-skill]'); if (addButton) addPlayerToLobby(addButton.getAttribute('data-add-db')); if (removeButton) removePlayerFromDatabase(removeButton.getAttribute('data-remove-db')); if (editSkillButton) openSkillModal(editSkillButton.getAttribute('data-edit-skill')); });
