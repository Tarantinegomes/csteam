...
const MAPS = [
  ...
  { id: 'anubis', name: 'Anubis', image: 'maps/anubis.webp', logo: 'maps/anubis.webp', background: 'maps/backgroundmaps/Anubis1.webp', description: 'Água, entrada forte e rounds rápidos com muita pressão de mapa.' },
...
async function handleSteamRegister() { const steamUrl = normalizeSteamUrl(steamProfileInput.value); const skill = normalizeSkill(playerSkillInput.value); if (!steamUrl) { setSummary('Cole um link válido do perfil Steam.'); return; } addPlayerBtn.disabled = true; addPlayerBtn.textContent = 'Carregando...'; try { const profile = await resolveSteamProfile(steamUrl); steamPreviewData = profile; ensureSteamPreview(profile); const action = await addPlayerToDatabase(profile, skill); renderDatabasePlayers(); renderRanking(); setSummary(action === 'updated' ? `${profile.personaName} já existia e foi atualizado.` : `${profile.personaName} foi cadastrado globalmente.`); steamProfileInput.value = ''; playerSkillInput.value = ''; } catch (error) { setSummary(error.message || 'Falha ao carregar o perfil Steam.'); } finally { addPlayerBtn.disabled = false; addPlayerBtn.textContent = 'Cadastrar'; } }
